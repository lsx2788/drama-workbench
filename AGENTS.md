...
- The preference catalog is shared by the UI and deterministic validation. Users opt into categories and then choose their options; unselected categories must not enter the saved brief or coordinator message. Reject duplicate categories, unknown or cross-category options and stale hidden custom values. Preserve legacy style records through migrations.
