...
for (const policy of [
  {
    ...
    version: 15,
    instructions: CHILD_COLLABORATION_INSTRUCTIONS,
    migrate: migrateChildCollaboration,
  },
  {
    name: "coordinator handoff",
    version: 16,
    instructions: COORDINATOR_HANDOFF_INSTRUCTIONS,
    migrate: migrateCoordinatorHandoff,
  },
])
  test(`${policy.name} upgrades coordinators once without changing existing child prompts or history`, (t) => {
    const s = setup(t),
      a = project(s);
    assert.ok(
    ...
    s.run("DELETE FROM schema_migrations WHERE version=?", policy.version);
    policy.migrate(s);
