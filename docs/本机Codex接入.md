...
- 内部上下文说明放在系统指令字段，不作为用户消息发送。后台 Thread 明确命名为「映序后台 · 项目 · AI」；完成后通过 Codex 的归档接口收起，下次恢复原 Thread 再继续。运行时可能短暂出现在 Codex 侧栏，归档不删除工作台消息、资产或原生历史。连接中断无法收起时保留记录，不能为清理侧栏自动重发任务。
